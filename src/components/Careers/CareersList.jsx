      ...
      >
        ...
        <div className="info">
          ...
          <h3 className="title" style={{ fontFamily: "Inter" }}>
            <Link to={process.env.PUBLIC_URL + `/careers-details/${data?.id}`}>
              {data.jobTitle}
            </Link>
          </h3>
          <ul className="meta">
            ...
            <li>({data?.noOfOpenings} Openings)</li>
          </ul>
          <Link
            to={process.env.PUBLIC_URL + `/careers-details/${data?.id}`}
            className="link"
          >
            {" "}
            <mark>Apply Now</mark>{" "}
...
CareersList.propTypes = {
  data: PropTypes.shape({
    ...
    noOfOpenings: PropTypes.string.isRequired,
  }).isRequired,
};
